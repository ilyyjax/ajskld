...
document.addEventListener('keyup', e => {
  if (e.code === 'Space') {
    holding = false;
    showEndScore(score); // show score popup
    score = 0;
    pickleY = 20;
    multiplier = 1;
    lastMilestone = 0;
...
function spawnRainbow(y) {
  if (Math.random() > 0.002) return; // 0.2% chance
  ...
  rainbow.dataset.speed = 1; // match cloud speed
  shakeCamera(150, 3); // small shake on rainbow
}
...
function shakeCamera(duration = 200, intensity = 5) {
  const gameAreaStyle = gameArea.style;
  let start = Date.now();
  ...
  function shake() {
    ...
    if (elapsed < duration) {
      ...
      gameAreaStyle.transform = `translate(${x}px, ${y}px)`;
      requestAnimationFrame(shake);
    } else {
      gameAreaStyle.transform = `translate(0,0)`;
    }
  }
  shake();
}
